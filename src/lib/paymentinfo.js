...
export async function decreasePayment(token,paymentId) {
    try {
        ...
        const response = await axios.patch(endpoint + 'payments/dec/'+paymentId, null, {
...
export async function increasePayment(token,paymentId) {
    try {
        // เก็บ token ลงใน Cookies (client-side)
        Cookies.set('token', token, { expires: 1 });
        console.log("token??")
        console.log(token)
        // ส่งคำขอ GET พร้อม cookie ที่มี token
        const response = await axios.patch(endpoint + 'payments/inc/'+paymentId, null, {
            withCredentials: true, // อนุญาตให้ส่ง cookie ไปยัง backend
        });
